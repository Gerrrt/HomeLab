...
| **The backup sets** — the firewall export, the volume sets, the NAS set | `backups/` on `prometheus`; the same on `oracle`, hashed there every morning; and the newest of each kind on the medium that holds the second age recipient — so whoever holds that key holds the sets it opens ([ADR-0047](../adr/0047-carry-the-estates-backup-sets-with-the-second-recipient.md), [`copy-the-backups-offsite.md`](copy-the-backups-offsite.md)) | **Never.** Ciphertext to the estate's age recipients everywhere it exists; `backups/` is gitignored and CI asserts it |
