...
- Jellyfin's and Navidrome's admins on `smaug`, and TrueNAS's own web-UI
  ...
  RFC 1918 range is until its *LAN networks* setting says otherwise.
  Navidrome's is reset from the same TrueNAS shell, and prompts for the new
  password:
  ...
  TrueNAS's admin has no such path from here; get it from the outgoing
  operator.
