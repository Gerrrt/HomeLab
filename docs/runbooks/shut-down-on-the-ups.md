...
has to cover. The Proxmox host firewall is disabled as of 2026-09-20
([#566](https://github.com/Gerrrt/HomeLab/issues/566)); when it is enabled,
its outbound policy is *accept* by default and this client needs nothing from
it.
...
it is a decision for an amendment to ADR-0047 rather than a line here.
...
| [ADR-0047](../adr/0047-shut-down-on-the-ups-from-a-nut-server-on-the-firewall.md) | Its "a configuration nobody has tested" line is amended with the dates, per ADR-0001 — a `> [!NOTE]` block, not an edit |
