...
Navidrome's state out of the dataset's newest ZFS snapshot, encrypts it here, and then copies
...
| `backup-nas` exits 1 | The message says which: *cannot reach* means SSH is off on `smaug`, the `99 → 40:22` pass is out of position, or the key or host key is missing; *newest snapshot … is N hours old* means the periodic task on `smaug` has stopped; *tar could not read* means Jellyfin or Navidrome wrote a file `frodo` cannot read; *cannot read … the directory does not exist* names a service whose directory is not in that snapshot — Navidrome not yet deployed (§6.5) | Nothing is stopped on either side and the last complete set is intact. [`build-the-nas.md`](build-the-nas.md) §6.2 is the setup this checks against, and names the fallback for the third case. A snapshot named in the *future* means `NAS_SNAPSHOT_TZ` is not `smaug`'s zone |
