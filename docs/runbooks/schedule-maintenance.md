...
| Every `homelab_job_*` series missing, no scrape error | The `textfile` block in `config.alloy` is not reading the right path | It must be `/rootfs`-prefixed. `rootfs_path` does **not** apply to that argument, and a wrong path reports an empty directory rather than an error |
