...
| An attacker on the lab segment reaching the hypervisor's management plane | The Proxmox firewall on `Saruman` admits `8006`, `8007` and `22` from Hicks and the API from `phoenix`, and nothing else from the segment ([ADR-0014](adr/0014-put-ifrit-on-imaginationlan-and-give-the-targets-no-route.md), [ADR-0043](adr/0043-keep-the-ca-on-prometheus-and-build-phoenix-as-the-deployment-host.md)). It was found disabled and turned on on 2026-09-20 ([#566](https://github.com/Gerrrt/HomeLab/issues/566)). `PveFirewallDisabled` pages if it is stopped or its inbound policy is left at `ACCEPT`, read from `homelab_pve_firewall_enabled` and `homelab_pve_firewall_policy_drop` on the host ([#576](https://github.com/Gerrrt/HomeLab/issues/576)) |
...
| Losing visibility of a failure | 119 alert rules, 30 days of metrics and logs |
...
visit, from that medium's first visit onward ([ADR-0048](adr/0048-carry-the-estates-backup-sets-with-the-second-recipient.md)). That medium
...
- **SNMPv3 authPriv, per poll**
  ...
  reads the community every minute. **`shiva` has been polled over v3 with
...
- **Outbound probes leave VLAN 99 for the public internet**, and that is a
  deliberate new egress path rather than a side effect
  ([#166](https://github.com/Gerrrt/HomeLab/issues/166)). blackbox-exporter
  makes TCP connects to `1.1.1.1:443` and `8.8.8.8:53` every 30 seconds so the
  estate can answer "is the internet slow, and is it us or the ISP". They are
  connects only — nothing is sent, nothing about the estate is disclosed beyond
  the fact that this address exists, which the ISP knows anyway. `tcp_connect`
  rather than an ICMP prober specifically so that blackbox-exporter keeps
