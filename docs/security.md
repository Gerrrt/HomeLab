...
| An attacker on the lab segment reaching the hypervisor's management plane | `Saruman`'s Proxmox firewall admits `8006`, `8007` and `22` from VLAN 50 only, plus the deployment host to `8006` ([ADR-0014](adr/0014-put-ifrit-on-imaginationlan-and-give-the-targets-no-route.md), [ADR-0043](adr/0043-keep-the-ca-on-prometheus-and-build-phoenix-as-the-deployment-host.md)). It was found **off** and enabled on 2026-09-20 ([#566](https://github.com/Gerrrt/HomeLab/issues/566)). `scripts/collect-pve-firewall.sh` reports its state, and `PveFirewallDisabled` pages if it goes off again, but only once the collector is installed on the host ([#576](https://github.com/Gerrrt/HomeLab/issues/576)) |
