...
88 rules in total: 72 metric-based in `prometheus/rules/`, and 16 log-based in
`loki/rules/`.
...
72 rules across eleven files in `prometheus/rules/`:
...
against the broken rule too. Coverage is fifty-one rules of 72 so far — the five
...
`SuricataStopped`, and all fifteen in `host.rules.yaml` —
