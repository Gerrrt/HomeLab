...
90 rules in total: 74 metric-based in `prometheus/rules/`, and 16 log-based in
`loki/rules/`.
...
74 rules across eleven files in `prometheus/rules/`:
...
against the broken rule too. Coverage is fifty-three rules of 74 so far — the five
...
[#353](https://github.com/Gerrrt/HomeLab/issues/353), and all fifteen in
`host.rules.yaml` —
