...
Five services. Two are plumbing; Home Assistant and Vaultwarden are the first
household services and the shape every later one takes; AdGuard is the one the
household uses without ever knowing it. What is absent is as deliberate as what is
here:
...
- **No other services yet.** Immich, Paperless-ngx, ntfy and Homepage
  ([#132], [#133], [#136], [#137]) each arrive as Home Assistant and
  ...
  — AdGuard is the single argued exception, and `compose.yaml` makes the
  argument at DIFFERENCE 7 so that the next one has to be made too.
...
- **Every routed name needs a SAN and a host override.** The `Caddyfile`
  ...
  and `vaultwarden.matrix.elysium` have to be on the leaf
...
volume it cannot verify, so each of the tier's six has a sentinel entry there
...
for Home Assistant, `data/stats.db` for AdGuard — and `restore-volumes.sh` knows
the uid each must come back
...
- **That the leaf matches the names.** `caddy validate` loads a throwaway
  ...
  `vaultwarden.matrix.elysium` in its SANs is checked by the first browser,
